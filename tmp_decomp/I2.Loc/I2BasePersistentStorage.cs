...
public abstract class I2BasePersistentStorage
{
	public virtual void SetSetting_String(string key, string value)
	{
		try
		{
			...
			{
				int num3 = num * i;
				PlatformPlayerPrefs.SetString($"[I2split]{i}{key}", value.Substring(num3, Mathf.Min(num, length - num3)));
			}
			PlatformPlayerPrefs.SetString(key, "[$I2#@div$]" + num2);
		}
		catch (Exception)
		{
	...
	public virtual string GetSetting_String(string key, string defaultValue)
	{
		try
		{
			string text = PlatformPlayerPrefs.GetString(key, defaultValue);
			if (!string.IsNullOrEmpty(text) && text.StartsWith("[I2split]", StringComparison.Ordinal))
			{
				...
				for (int i = 0; i < num; i++)
				{
					text += PlatformPlayerPrefs.GetString($"[I2split]{i}{key}");
				}
			}
			return text;
		}
	...
	public virtual void DeleteSetting(string key)
	{
		try
		{
			string text = PlatformPlayerPrefs.GetString(key, null);
			if (!string.IsNullOrEmpty(text) && text.StartsWith("[I2split]", StringComparison.Ordinal))
			{
				int num = int.Parse(text.Substring("[I2split]".Length), CultureInfo.InvariantCulture);
				for (int i = 0; i < num; i++)
				{
					PlatformPlayerPrefs.DeleteKey($"[I2split]{i}{key}");
				}
			}
